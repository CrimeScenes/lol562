...
local function getDynamicFov(targetPosition)
    ...
    -- Check if the target is within the Close range (0 to CloseDistance)
    if distance <= getgenv().Ranges.CloseDistance then
        return getgenv().Silent.AimSettings.FovSettings.CloseFovRadius
    -- Check if the target is within the Mid range (CloseDistance to MidDistance)
    elseif distance > getgenv().Ranges.CloseDistance and distance <= getgenv().Ranges.MidDistance then
        return getgenv().Silent.AimSettings.FovSettings.MidFovRadius
    -- Check if the target is within the Far range (MidDistance to FarDistance)
    elseif distance > getgenv().Ranges.MidDistance and distance <= getgenv().Ranges.FarDistance then
        return getgenv().Silent.AimSettings.FovSettings.FarFovRadius
    else
        -- Default to Far FOV if the target is beyond FarDistance
        return getgenv().Silent.AimSettings.FovSettings.FarFovRadius
    end
end
...
local function updateFov()
    ...
    clearFovParts()
    if IsTargeting and TargetPlayer then  
        -- Calculate dynamic FOV radius based on distance to target
        local targetDistance = getDynamicFov(closestPoint)

        -- Update FOV display shape based on the dynamic radius
